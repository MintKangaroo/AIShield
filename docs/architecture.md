...
현재 release는 PyTorch 이미지 분류 모델의 재현 가능한 clean/FGSM/BIM/PGD 평가를 제공합니다.
        ...
        └─> nginx /api proxy
              └─> FastAPI
                    └─> RegistryService (process-local handles and run index)
                          ...
                          └─> FGSM / BIM / PGD engine ─> paired attack evidence
...
- `aishield.attacks` — framework-independent attack contract와 bounded FGSM/BIM/PGD runner.
...
compatibility and [0,1] validation
  ...
  -> input gradient
  -> FGSM step, iterative BIM, or randomized PGD
