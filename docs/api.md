...
| `POST` | `/api/v1/registry/attacks` | Bounded FGSM/BIM/PGD/DeepFool run |
...
지원 algorithm은 `fgsm`, `bim`, `pgd`, `deepfool`입니다. FGSM/BIM/PGD는 `linf`, DeepFool은
`l2` norm을 사용합니다.
...
`step_size=epsilon`, `iterations=20`, `random_start=false`입니다.
